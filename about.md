...
* 継続的に選手と話し検証を繰り返しながらサービスの改善につなげる
...
* Ruby on Rails + Chef によるサーバ自動セットアップWeb APIサーバを開発
...
* 3D仮想空間コミュニティー(メタバース)サービス Meet-Me(http://www.meet-me.jp/) 立ち上げのためにジョイン。東京の実際の地形データを用いて東京を仮想空間に再現するコンセプト。ネットワーク/サーバ/運用監視システムを一人で構築し、構想から実質4ヶ月でサービスインさせる荒業を経験
    ...
    * サーバOSのインストール・セットアップ、ミドルウエアインストール・セットアップ、サーバアプリケーションのデプロイ。テスト環境と本番環境の同一環境(計30台のサーバ)を用意。
    * 各サーバの監視・モニタリングサーバのセットアップ。障害時に携帯電話に通知する監視システムの構築。
    * サーバ台数60台への拡張計画の策定・拡張作業のメンテナンス実施も行う。
